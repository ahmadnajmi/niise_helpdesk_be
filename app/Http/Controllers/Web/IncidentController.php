...
class IncidentController extends Controller
{
    ...
    public function generateDueDateIncident(Request $request){
        ...
        $data['sla_version_id'] = $request->sla_version_id;
        $data['incident_date'] = $incident_date;
        $data['branch_id'] = $request->branch_id;

        if($data['sla_version_id'] &&  $data['branch_id']){
            $generate_due_date = IncidentServices::calculateDueDateIncident($data);
        }

        $list_sla_version = SlaVersion::get();
        ...
        return view('incident.generate_duedate',compact('generate_due_date','list_branch','list_sla_version','operating_time','public_holiday','incident_date'));
    }
}
